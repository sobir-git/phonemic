...
class TrackpadTests(unittest.IsolatedAsyncioTestCase):
    async def test_commands(self):
        ...
        with patch.dict(webmic.os.environ, {'DISPLAY': ':0', 'XDG_SESSION_TYPE': 'x11'}), patch.object(webmic.asyncio, 'create_subprocess_exec', return_value=proc) as spawn:
    ...
    async def test_reject_invalid_commands(self):
        with patch.object(webmic.asyncio, 'create_subprocess_exec') as spawn:
    ...
    async def test_wayland_error(self):
        with patch.dict(webmic.os.environ, {'XDG_SESSION_TYPE': 'wayland'}), self.assertRaisesRegex(RuntimeError, 'X11'):
